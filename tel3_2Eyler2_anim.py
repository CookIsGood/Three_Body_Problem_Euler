...
eq1 = (M0+M2)*a**5+(3*M0+2*M2)*a**4+(3*M0+M2)*a**3-(M2+3*M1)*a**2-(2*M2+3*M1)*a-(M2+M1)
solve_a=solve(eq1, a)
A=solve_a[0]

nu2=f1*((M0+M2)-M1/(A)**2+M1/(A+1)**2)
c=200000
po=c**2/nu2
e=0.5
p=po/(1+e)
...
for i in range(len(t)):
    ...
    plt.plot((xx0[i], xx1[i], xx2[i], xx0[i]), (yy0[i], yy1[i], yy2[i], yy0[i]), linewidth=1, color='magenta')
    ax_1.add_patch(pc1)
    ax_1.add_patch(pc2)
    ax_1.add_patch(pc3)
